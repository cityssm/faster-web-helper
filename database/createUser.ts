...
import { generateKeyGuid } from '../helpers/users.helpers.js'

import { databasePath } from './helpers.userDatabase.js'

export default function createUser(
  user: Partial<FasterWebHelperSessionUser> & { userName: string }
): boolean {
  ...
  const userRecord = database
    ...
    .get(user.userName) as { recordDelete_timeMillis: number | null } | undefined
  ...
  if (userRecord === undefined) {
    database
      ...
      .run(
        ...
        user.emailAddress,
        user.userKeyGuid ?? generateKeyGuid(),
        user.userName,
        rightNow,
        user.userName,
        rightNow
