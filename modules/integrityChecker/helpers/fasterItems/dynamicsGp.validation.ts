...
export async function updateInventoryInFaster(): Promise<void> {
  /*
   * Declare FASTER APIs
   */
  ...
  const fasterUnofficialAPI = new FasterUnofficialAPI(
  ...
  const fasterAPI = new FasterApi(
    fasterWebConfig.tenantOrBaseUrl,
    fasterWebConfig.apiUserName as string,
    fasterWebConfig.apiPassword as string
  )
  ...
  for (const record of recordsToUpdate) {
    ...
    if (record.fasterItemName === null) {
      ...
      debug(
        `Update FASTER item "${record.itemNumber} [${record.storeroom}]" bin to "${notFoundInDynamicsGpBinLocation}"...`
      )
