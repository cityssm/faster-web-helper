...
async function refreshFasterVendors(): Promise<void> {
  if (
  ...
  debug(
    `Updating ${fasterVendorsResponse.response.results.length} FASTER vendors...`)

  /*
   * Update FASTER vendors
   */

  const updateSource = getConfigProperty(
    'modules.integrityChecker.fasterVendors.update.source'
  )

  if (updateSource === 'dynamicsGp') {
    const dynamicsGpUpdate = await import(
      '../../helpers/fasterVendors/dynamicsGp.vendorUpdate.js'
    )

    await dynamicsGpUpdate.updateVendorsInFaster(
      fasterVendorsResponse.response.results
    )
  } else {
