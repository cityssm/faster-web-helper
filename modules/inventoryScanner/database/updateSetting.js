...
export default function updateSetting(settingName, settingValue, retriesRemaining = maxRetries) {
    if (retriesRemaining <= 0) {
        debug(`Failed to update setting "${settingName}" with value "${settingValue}" after multiple attempts.`);
        return false;
    }
    ...
    try {
        ...
        debug(`Error updating setting "${settingName}" with value "${settingValue}": ${error}`);
